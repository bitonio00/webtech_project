...
export default forwardRef(
  ({ channel, onScrollDown, messages, setMessages }, ref) => {
    const { oauth, avatarUser, gravatar, usersChannel, setUsersChannel } =
      useContext(Context);
    ...
    const onDelete = async (message) => {
      const res = await axios.delete(
        `http://localhost:3001/channels/${message.channelId}/messages/${message.creation}`,
        {
          headers: {
            Authorization: `Bearer ${oauth.access_token}`,
          },
    ...
    const onEdit = async () => {
      const res = await axios.put(
        `http://localhost:3001/channels/${editedChannelId}/messages/${editedCreation}`,
        { content: editContent },
        {
          headers: {
            Authorization: `Bearer ${oauth.access_token}`,
